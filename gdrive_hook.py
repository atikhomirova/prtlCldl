...
class GoogleDriveHook(GoogleCloudBaseHook, LoggingMixin):
    ...
    def get_file_IDs_list(self):
        ...
        while True:
            ...
            for file in response.get('files', []):
                ...
                if file.get('mimeType') in self.valid_mimeTypes:
                    fileIDsList = fileIDsList + file.get('id') + ','
        ...
        if fileIDsList == '':
            ...
            fileIDsList = fileIDsList[:-len(',')]
        return fileIDsList


    def get_file(self, file_id):
        service = self.get_service()
        file_mimeType = service.files().get(
            fileId=file_id).execute().get('mimeType') # removed supportsTeamDrives=True from get arguments
        logging.info('File mime type is:', file_mimeType)
        file_name = service.files().get(
            fileId=file_id).execute().get('name')
        ...
        logging.info('File ' + file_name + self.targ_file_name + ' is loaded')
