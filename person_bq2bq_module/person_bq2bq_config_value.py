            ...
            for function in functions:
                ...
                if value == 'FILTERED':
                    return [beam.pvalue.TaggedOutput(
                        cls.SIDE_OUTPUT_TAG_FILTERED_ROW, row)]
                else:
                    dct[name] = value
        return [dct]
...
def run(argv=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        config = config_list | 'ConfigTransform' >> beam.ParDo(Config())
        ...
        transformed = lines | 'DataTransform' >> beam.ParDo(
            ApplyConfig(), beam.pvalue.AsList(config)).with_outputs(
               ApplyConfig.SIDE_OUTPUT_TAG_FILTERED_ROW, main='main')
